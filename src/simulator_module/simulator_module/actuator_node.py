...
class ActuatorNode(Node):
    ...
    def __init__(self, ship_number):
        ...
        super().__init__("actuator", namespace="ship"+str(ship_number))
        self.n_p=0.0
    ...
    def actuated(self,n_p,rudder_angle,delta_time):
        ...
        if abs(d_n_p)<n_p_rate_max*delta_time:
            res_n_p=n_p
        else:
            if d_n_p>0:
                res_n_p=current_n_p+n_p_rate_max*delta_time
            else:
                res_n_p=current_n_p-n_p_rate_max*delta_time

        current_angle=self.actuated_list['rudder_angle'][-1]
        d_angle=rudder_angle-current_angle
        if abs(d_angle)<rudder_rate_max*delta_time:
            res_angle=rudder_angle
        else:
            if d_angle>0:
                res_angle=current_angle+rudder_rate_max*delta_time
            else:
                res_angle=current_angle-rudder_rate_max*delta_time
